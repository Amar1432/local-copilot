#!/usr/bin/env node
/**
 * Standalone CLI runner for Private Copilot benchmarks.
 *
 * Usage:
 *   node --loader ts-node/esm benchmarks/run-benchmark.ts [--model <name>] [--endpoint <url>] [--lang <langs>]
 */

import { BenchmarkRunner, DEFAULT_BENCHMARK_DATASET, type CompletionExecutor } from "@private-copilot/core";
...
async function main() {
  ...
  console.log(`[Private Copilot] Starting evaluation benchmark...`);
  console.log(`[Private Copilot] Target Model: ${modelArg}`);
  console.log(`[Private Copilot] Endpoint: ${endpointArg}`);
...
main().catch((err) => {
  console.error("[Private Copilot Benchmark] Error:", err);
  process.exit(1);
});
